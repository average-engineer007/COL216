...
int main() {
    ...
    std::vector<int> cache_sizes = {1,2,4,8,32,64, 128, 256, 512,1024};  // Cache sizes in KB
    std::vector<int> associativities = {1, 2, 4, 8,16,32,64,128,256,512,1024};     // Associativity
    std::vector<int> block_sizes = {4,8,16,};    // Block sizes in bytes
    ...
    for (int cache_size : cache_sizes) {
        for (int associativity : associativities) {
            for (int block_size : block_sizes) {
                // Iterate over each configuration of write policy, write strategy, and replacement policy
                for (const std::string& write_policy : write_policies) {
                    for (const std::string& write_strategy : write_strategies) {
                        for (const std::string& replacement_policy : replacement_policies) {
                            // Run cacheSim executable with the current configuration
                            // cout<<"Yo"<<endl;
                            ...
                            stats.push_back(cache_size * block_size * associativity); // total cache size
                            double current_val = stats[0]+(0.1)*stats[2];

                            // Write stats to the analysis file
                            // analysis_file << "Configuration: " << cache_size << "KB " << associativity << "-way " << block_size << "B " << write_policy << " " << write_strategy << " " << replacement_policy << std::endl;
                            ...
                            if (current_val < best_val) { // Check hit rate
    ...
    std::cout << "Cache Size: " << best_configuration[0] << " sets" << std::endl;
